...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    addr32 base_v_addr = (virtual_address >> NUM_OFFSET_BITS) << NUM_OFFSET_BITS;
    page_table_entry *pte = tlb_get_pte(&this->tlb, base_v_addr);
    //If it’s not there
    if (!pte) {
        //Raise a TLB miss
        mmu_tlb_miss(this);
        ...
        addr32 v_first_addr = virtual_address >> (NUM_VPN_BITS + NUM_OFFSET_BITS);
        page_directory_entry* pde = &pd->entries[v_first_addr];
        ...
        addr32 v_second_addr = (virtual_address >> (NUM_OFFSET_BITS)) & 0X3FF;
        pte = &pt->entries[v_second_addr];
        tlb_add_pte(&this->tlb, base_v_addr, pte);
    } 
    ...
    addr32 phy_addr = (pte->base_addr << NUM_OFFSET_BITS) | (virtual_address & 0XFFF);
    void* phy_addr_ptr = get_system_pointer_from_address(phy_addr);

    //Perform the read or write operation
    memcpy(buffer, phy_addr_ptr, num_bytes);
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    //Perform the read or write operation
    memcpy(phy_addr_ptr, buffer, num_bytes);
